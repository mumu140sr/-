/* ===========================================
   ...
   =========================================== */
self.importScripts('data.js?v=96', 'optimizer.js?v=96', 'milp-core.js?v=96');
...
function getSolver() {
  if (!_solverPromise) {
    self.importScripts(HIGHS_BASE + 'highs.js?v=96'); // → self.Module（Emscripten factory）
...
self.addEventListener('message', async (e) => {
  ...
  try {
    ...
    for (const g of groups) {
      ...
      const timeLimit = Math.max(15, Math.min(540, parseInt((AppState.settings || {}).milpTimeLimit) || 90));
